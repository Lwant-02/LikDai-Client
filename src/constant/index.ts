export const developerContacts = [
  {
    imagePath: "/svg/facebook.svg",
    link: "https://www.facebook.com/share/1KAi1dVAsy/?mibextid=wwXIfr",
  },
  {
    imagePath: "/svg/instagram.svg",
    link: "https://www.instagram.com/lwant02?igsh=NHZmYWVuenJtNmZ2&utm_source=qr",
  },
  {
    imagePath: "/svg/linkedin.svg",
    link: "https://www.linkedin.com/in/sai-naw-main-03b655281/",
  },
  { imagePath: "/svg/github.svg", link: "https://github.com/Lwant-02" },
];

export const supporterContacts = [
  {
    imagePath: "/svg/facebook.svg",
    link: "https://www.facebook.com/saing.hmine.tun",
  },
  { imagePath: "/svg/github.svg", link: "https://github.com/SaingHmineTun" },
];

export const anotherSupporterContacts = [
  {
    imagePath: "/svg/facebook.svg",
    link: "https://www.facebook.com/share/1JyUNRQo6H/?mibextid=wwXIfr",
  },
];

export const githubFrontendRepoUrl =
  "https://github.com/Lwant-02/LikDaiPro-Client.git";

export const githubBackendRepoUrl =
  "https://github.com/Lwant-02/LikDaiPro-Server.git";

export const SONGS = [
  {
    id: "1",
    title: "ၵိုတ်းဝႆႉၵေႃႉလဵဝ်",
    artist: "ဢိၼ်းၾၢင်",
    url: "/songs/kaw-lawe-inn-fung.mp3",
    color: "text-green",
    bg: "bg-green/10",
  },
  {
    id: "2",
    title: "ၶုၵ်းၸႂ်လႆႈမႂ်း",
    artist: "ဢိၼ်းၾၢင်",
    url: "/songs/khut-ja-lai-inn-fung.mp3",
    color: "text-green",
    bg: "bg-green/10",
  },
  {
    id: "3",
    title: "ငဝ်းႁၵ်ႉၽၢမ်းတႃ",
    artist: "ၵဵင်ၶမ်း",
    url: "/songs/pham-tar-kein-kham.mp3",
    color: "text-green",
    bg: "bg-green/10",
  },
  {
    id: "4",
    title: "ယွၼ်ႉႁၵ်ႉ",
    artist: "ဢိၼ်းၾၢင်",
    url: "/songs/yount-hak-inn-fung.mp3",
    color: "text-green",
    bg: "bg-green/10",
  },
  {
    id: "5",
    title: "ယုမ်ႇလႃႈ",
    artist: "လိူၼ်ၽူး",
    url: "/songs/yum-la-lain-phoo.mp3",
    color: "text-green",
    bg: "bg-green/10",
  },
  {
    id: "6",
    title: "Study With Me",
    artist: "Study Song",
    url: "/songs/study-with-me.mp3",
    color: "text-green",
    bg: "bg-green/10",
  },
];
